...
zero.main = (function() {
	...
	return {
		...
		registerEvents: function() {
			...
			zero.loading.init($('.page-case-study').length);
		...
		scrollBindings: function() {
			if($('.page-case-study').length) {
				if($(window).width() >= 1200 && !zero.main.isMobile) {
					zero.casestudy.playOrPauseHero();
					zero.casestudy.fadeInObjects();
					zero.parallax.doCaseStudy();
				}
			}
			else {
				if($(window).width() >= 1200 && $('.page-home').length == 0) {
					zero.parallax.doStandard();
				}
		...
		resizeBindings: function() {
			if($('.page-case-study').length) {
				if($(window).width() < 1200 || zero.main.isMobile) {
					zero.parallax.resetCaseStudy();
				}
			}
			else {
				if($(window).width() < 1200 || zero.main.isMobile) {
					zero.parallax.resetStandard();
				}
			}
		}
	};
...
zero.masthead = (function() {
	...
	return {
		init: function() {
			if ($("body.page-pix").length || $("div.no-header-image").length) {
	...
	return {
		drawTimer: function()
		{
			...
			var radius = 120, // circle radius relative to svg viewbox (be sure to account for stroke width)
					...
					ms = zero.feature.s.constants.pause - zero.feature.s.constants.speed - 400, // duration of animation (milliseconds)
			...
			timer = setInterval(
				function() {
					...
					if (i >= numTicks) {
						zero.timer.stopTimer();
						zero.feature.$bxapi.goToNextSlide();
					}
				}, interval);
		},
		stopTimer: function()
...
zero.feature = (function() {
	...
	return {
		...
		init: function()
		{
			var t = window.setTimeout(kickoff, zero.feature.s.constants.delay);
			function kickoff()
			{
				...
				var s = zero.feature.s;
				var $c = $("." + s.classes.i),
						$items = $c.find(".item");

				if ($items.length) {
					zero.feature.resizeItems();

                    $(window).on("throttledresize", function() {
                        zero.feature.resizeItems();
                    });

					zero.feature.enable(s, $c, $items);
				}
				// jQuery Color plugin
				$.Color.hook("fill");
				if(zero.main.isTouchDevice()) {
		...
		enable: function(s, $c, $items)
		{
			...
			zero.feature.$bxapi = $c.bxSlider({
				...
				onSliderLoad: function(currentIndex) {
					...
					zero.feature.setClient(s, currentIndex, $items);
					zero.feature.setActive(s, currentIndex, $pager, $items);
				},
				onSlideBefore: function($slideElement, oldIndex, newIndex) {
					zero.feature.hide(s, oldIndex, newIndex, $items, $pager);
					zero.feature.setClient(s, newIndex, $items);
					zero.timer.stopTimer();
					if(scrollTripped) inTransition = 1;
				},
				onSlideAfter: function($slideElement, oldIndex, newIndex) {
					zero.feature.setActive(s, newIndex, $pager, $items);
					setTimeout(function() {
						inTransition = 0;
					},1000);
				}
		...
		setClient: function(s, i, $items)
		{
			...
			if(newProject !== "zero") zero.video.pause();
			else zero.video.play();
		},
		setActive: function(s, i, $pager, $items)
		{
			...
			if(!scrollTripped) {
				...
				zero.timer.drawTimer();
			}

			zero.main.setLogoColors(s.colors[newProject.replace("-", "")], s.txt[newProject.replace("-", "")]);
			zero.feature.setPagerColor($pager, s.txt[newProject.replace("-", "")]);
...
zero.carousel = (function() {
	...
	return {
		...
		init: function() {
			...
			settings.$el.each(function() {
				...
				var $c = $(this),
						...
						enableTabControls = $m.hasClass("enable-tab-controls") && zero.main.windowWide();
				...
				var $bxapi = $c.bxSlider({
					...
					onSliderLoad: function(currentIndex) {
						...
						if (enableCaptions) {
							zero.carousel.insertCaptions($m, $c, $active);
						}

						// add controls
						zero.carousel.insertControls($c);
					},
					onSlideAfter: function($slideElement, oldIndex, newIndex) {
						...
						if (enableCaptions) {
							zero.carousel.updateCaption($m, $c, $slideElement);
						}
						if (enableTabControls) {
							zero.carousel.setActiveTab($c, $slideElement);
						}
					}
				});

				// enable controls
				zero.carousel.bindControls($c, $bxapi);
				enableTabControls
						? zero.carousel.bindTabControls($c, $bxapi)
						: null;
			});
		},
		...
		insertCaptions: function($m, $c, $active) {
			...
			zero.carousel.updateCaption($m, $c, $active);
		},
	...
	init: function()
	{
		...
		this.settings.$el.each(function() {
			...
			if (zero.main.windowWide()) {
				...
				zero.tabs.upgradeTags($t, isTeam);
				zero.tabs.enableModule($t, moduleType, isTeam);
				zero.tabs.buildSelectors($t, moduleType, isTeam);
				zero.tabs.pageLoadActiveItem($t.not('[data-not-expanded]'), moduleType, isTeam);
				zero.tabs.bindUIfunctions($t, moduleType, isTeam);
			}
			// "small" screen & transform to accordion
				...
				zero.tabs.enableModule($t, moduleType, isTeam);
				zero.tabs.buildSelectors($t, moduleType, isTeam);
				zero.tabs.pageLoadActiveItem($t, moduleType, isTeam);
				zero.tabs.bindUIfunctions($t, moduleType, isTeam);
			}
		});
	},
	...
	buildSelectors: function($t, moduleType, isTeam)
	{
		if (moduleType == "tabs") {
			if(isTeam) {
				...
				// build
				$t.find("[data-tab-selector]").appendTo(zero.tabs.settings.$selectorWrap);
				// insert
				zero.tabs.settings.$selectorWrap.insertAfter($t.find(".module-heading").first());
			}
		}
		...
		if (moduleType == "tabs") {
			$t.children(".tab-selectors")
				...
				.on("click", "a[href^='#']", function(e) {
					if (zero.main.windowWide()) {
						zero.tabs.changeTab($t, this.hash, isTeam, $(this).hasClass('active'));
					}
				});
		}
		...
		if (moduleType == "accordion") {
			$t.find(".tab-item .accordion-tab")
				.on("click", function(e) {
					...
					zero.tabs.toggleAccordion($t, $tabItem);
				});
		}
	},
...
zero.pillTabs = {
	...
	init: function()
	{
		...
		this.settings.$el.each(function() {
			...
			zero.pillTabs.enableModule($t);
			zero.pillTabs.bindUIfunctions($t);
			zero.pillTabs.pageLoadActiveItem($t);
		});
	},
	...
	bindUIfunctions: function($t, moduleType)
	{
		$t.children(".pill-tab-selectors")
				...
				.on("click", "a[href^='#']:not('.active')", function(e) {
					zero.pillTabs.changeTab($t, this.hash);
				});
	},
...
zero.basicTabs = {
	...
	init: function()
	{
		...
		this.settings.$el.each(function() {
			...
			zero.basicTabs.enableModule($t, moduleType);
			zero.basicTabs.bindUIfunctions($t, moduleType);
			zero.basicTabs.pageLoadActiveItem($t, moduleType);
		});
	},
	enableModule: function($t, moduleType)
	{
	...
	bindUIfunctions: function($t, moduleType)
	{
		$t.find(".tabs-" + moduleType + "-selectors")
				...
				.on("click", "a[href^='#']:not('.active')", function(e) {
					zero.basicTabs.changeTab($t, this.hash);
				});
	},
	...
	return {
		init: function() {
			settings.$el.each(function() {
				...
				if (zero.main.windowWide()) {
					zero.teleport.enableModule($m);
					zero.teleport.bindUIfunctions($m);
				} else {
		...
		changeTab: function($m, hash)
		{
			...
			zero.teleport.insertNodes($newTextHolder);
	...
	return {
		init: function() {
			...
			settings.$el.each(function() {
				...
				zero.modal.bindUIfunctions();
				zero.modal.insertModal($m);
			});

			// close modal with esc
			$(document).keydown(function(e) {
				if (e.keyCode == 27) {
					zero.modal.closeModal($(".m_modal"));
				}
			});
		},
		...
		bindUIfunctions: function() {
			...
			$(".show-modal").on("click", function(e) {
				e.preventDefault();
				zero.modal.openModal($("#" + this.hash.replace("#", "")));
			});
			$(".m_modal").on("click", ".close", function(e) {
				e.preventDefault();
				zero.modal.closeModal($(this).closest(".m_modal"));
			});
		},
...
zero.casestudy = (function() {
	...
	return {
		init: function() {
			$(window).on('resize', function() {
				...
				rTimer = setTimeout(function() {
					zero.casestudy.setJumbotronSize();
				}, 60);
			});
		...
		setJumbotronSize: function() {
			var h = $(window).height();
			if($('.page-case-study').length && $(window).width() >= 1200 && $(window).height() >= 800 && !zero.main.isMobile) {
				h = $(window).height() - 150;
			}
...
jQuery(function() {
	...
	$(window).scroll(function() {
		zero.main.scrollBindings();
	});
	$(window).resize(function() {
		zero.main.resizeBindings();
	})
});
